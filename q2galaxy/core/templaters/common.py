...
def make_requirements(environment, *project_names):
    ...
    if environment.container is not None:
        requirements.append(
            XMLNode('container', environment.container, type='docker'))
    elif environment.metapackage is None:
        requirements.append(
            XMLNode('requirement', 'q2galaxy',
                    type='package', version=q2galaxy.__version__))

        for dep, version in environment.iter_deps(*project_names,
                                                  include_self=True):
            r = XMLNode('requirement', dep, type='package', version=version)
            requirements.append(r)
    else:
        requirements.append(
            XMLNode('requirement', environment.metapackage_name,
                    type='package', version=environment.metapackage_version))
